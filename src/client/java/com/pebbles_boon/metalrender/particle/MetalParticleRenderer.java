...
public class MetalParticleRenderer {
  ...
  public void captureParticles(ParticleEngine particleManager, Camera camera,
                               float tickDelta) {
    if (!active)
      return;
  ...
  public void captureParticleList(Queue<? extends Particle> particles,
    ...
    for (Particle p : particles) {
      ...
      cp.x = (float)(Mth.lerp(tickDelta, pa.metalrender$getLastX(),
                                     pa.metalrender$getX()) -
                     camX);
      cp.y = (float)(Mth.lerp(tickDelta, pa.metalrender$getLastY(),
                                     pa.metalrender$getY()) -
                     camY);
      cp.z = (float)(Mth.lerp(tickDelta, pa.metalrender$getLastZ(),
                                     pa.metalrender$getZ()) -
                     camZ);
      cp.scale = bp.getQuadSize(tickDelta);
      ...
      cp.zRotation =
          Mth.lerp(tickDelta, bpa.metalrender$getLastZRotation(),
                          bpa.metalrender$getZRotation());
      TextureAtlasSprite sprite = bpa.metalrender$getSprite();
      if (sprite != null) {
        cp.minU = sprite.getU0();
        cp.maxU = sprite.getU1();
        cp.minV = sprite.getV0();
        cp.maxV = sprite.getV1();
  ...
  private void captureFromManager(ParticleEngine manager, Camera camera,
                                  float tickDelta) {
  }
