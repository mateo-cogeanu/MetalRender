...
public class MetalWorldRenderer {
  ...
  private long metalFramesSubmitted;
  private long totalMetalChunksDrawn;
  private boolean lastFrameHadContext;
  ...
  public void beginFrame(Camera camera, float tickDelta, Matrix4f projection,
    ...
    lastFrameHadContext = frameCtx != 0;
    if (frameCtx != 0) {
      if (MetalRenderClient.getConfig().enableMetalRendering) {
        ...
        if (!skipTerrainDraw) {
          ...
          if (ibHandle != 0) {
            ...
            totalMetalChunksDrawn += Math.max(0, drawn);
  ...
  public void endFrame() {
    ...
    renderer.endFrame();
    if (lastFrameHadContext)
      metalFramesSubmitted++;
    frameCount++;
  }
  ...
  private void buildPendingChunkMeshes(Minecraft client) {
    scanForPendingChunks(client);
    if (client.player != null && chunkMesher.getMeshCount() < maxMeshes) {
      int playerChunkX = net.minecraft.core.SectionPos.blockToSectionCoord(client.player.getBlockX());
      int playerChunkZ = net.minecraft.core.SectionPos.blockToSectionCoord(client.player.getBlockZ());
  ...
  private void scanForPendingChunks(Minecraft client) {
        ...
        for (int dz = -ring; dz <= ring; dz++) {
          ...
          for (int sy = 0; sy < sections.length; sy++) {
            ...
            if (section == null || section.hasOnlyAir())
              continue;
            int worldY = (chunk.getMinSectionY() + sy);
      ...
      for (int dx = -ring; dx <= ring; dx++) {
        for (int dz = -ring; dz <= ring; dz++) {
          if (ring > 0 && Math.abs(dx) < ring && Math.abs(dz) < ring)
            continue;
          total++;
          if (world.getChunk(playerChunkX + dx, playerChunkZ + dz) != null) {
            available++;
            ringAvail++;
          }
        }
  ...
  private void rebuildLodMeshes(Minecraft client) {
  ...
  public long getMetalFramesSubmitted() {
    return metalFramesSubmitted;
  }
  public long getTotalMetalChunksDrawn() {
    return totalMetalChunksDrawn;
  }
  public boolean didLastFrameHaveMetalContext() {
    return lastFrameHadContext;
  }
  public long getSuccessfulCompositeCount() {
    return ioSurfaceBlitter.getSuccessfulCompositeCount();
  }
  public long getFailedCompositeCount() {
    return ioSurfaceBlitter.getFailedCompositeCount();
  }
  public long getFastPathCompositeCount() {
    return ioSurfaceBlitter.getFastPathCompositeCount();
  }
  public long getSlowPathCompositeCount() {
    return ioSurfaceBlitter.getSlowPathCompositeCount();
  }
  public String getLastCompositePath() {
    return ioSurfaceBlitter.getLastCompositePath();
  }
  private void pruneFarMeshes(Minecraft client,
  ...
  public void onChunkLoaded(int chunkX, int chunkZ, net.minecraft.world.level.chunk.LevelChunk chunk) {
