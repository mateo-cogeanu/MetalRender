...
public final class IOSurfaceBlitter {
  ...
  private long successfulCompositeCount = 0;
  private long failedCompositeCount = 0;
  private long fastPathCompositeCount = 0;
  private long slowPathCompositeCount = 0;
  private String lastCompositePath = "not attempted";
  ...
  public boolean blit(long metalHandle, boolean skipWait) {
    blitFrameCount++;
    if (metalHandle == 0) {
      failedCompositeCount++;
      lastCompositePath = "failed: no Metal handle";
      return false;
    }
    if (!initialized && !initialize()) {
      failedCompositeCount++;
      lastCompositePath = "failed: OpenGL compositor init";
      return false;
    }
    ...
    if (width <= 0 || height <= 0) {
      failedCompositeCount++;
      lastCompositePath = "failed: no IOSurface";
      return false;
    }
    ...
    boolean composited = blitGPUComposite(metalHandle, width, height, skipWait);
    if (composited) {
      successfulCompositeCount++;
    } else {
      failedCompositeCount++;
    }
    return composited;
    ...
    try {
      ...
      if (!ioSurfaceFailed &&
        ...
        if (bound && blitToIntermediateTexture(width, height)) {
          ...
          lastCompositePath = "IOSurface fast path";
          boolean drawn = drawFullscreenQuad(width, height);
          if (drawn)
            fastPathCompositeCount++;
          return drawn;
        } else {
      ...
      lastCompositePath = "CPU readback fallback";
      boolean drawn = blitSlowPath(metalHandle, width, height);
      if (drawn)
        slowPathCompositeCount++;
      return drawn;
    } catch (Exception e) {
      lastCompositePath = "failed: composite exception";
      if (blitFrameCount <= 10) {
  ...
  public long getAttemptedCompositeCount() {
    return blitFrameCount;
  }
  public long getSuccessfulCompositeCount() {
    return successfulCompositeCount;
  }
  public long getFailedCompositeCount() {
    return failedCompositeCount;
  }
  public long getFastPathCompositeCount() {
    return fastPathCompositeCount;
  }
  public long getSlowPathCompositeCount() {
    return slowPathCompositeCount;
  }
  public String getLastCompositePath() {
    return lastCompositePath;
  }
