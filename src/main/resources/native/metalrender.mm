...
static CAMetalLayer *g_directProbeLayer = nil;
static NSView *g_directProbeView = nil;
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_pebbles_1boon_metalrender_nativebridge_NativeBridge_nCreateDirectPresentationProbe(
  ...
  void (^createLayer)(void) = ^{
    ...
    if (!parent)
      return;
    if (g_directProbeView) {
      created = true;
      return;
    }
    g_directProbeView =
        [[NSView alloc] initWithFrame:NSMakeRect(12.0, 12.0, 196.0, 28.0)];
    [g_directProbeView setWantsLayer:YES];
    g_directProbeLayer = [[CAMetalLayer alloc] init];
    g_directProbeLayer.device = g_device;
    g_directProbeLayer.pixelFormat = MTLPixelFormatBGRA8Unorm;
    g_directProbeLayer.framebufferOnly = YES;
    g_directProbeLayer.opaque = YES;
    CGFloat scale = parent.window ? parent.window.backingScaleFactor : 1.0;
    g_directProbeLayer.contentsScale = scale;
    g_directProbeLayer.drawableSize = CGSizeMake(196.0 * scale, 28.0 * scale);
    g_directProbeView.layer = g_directProbeLayer;
    [parent addSubview:g_directProbeView];
    created = true;
  };
  if ([NSThread isMainThread])
    createLayer();
  else
    dispatch_sync(dispatch_get_main_queue(), createLayer);
  dbg("Direct CAMetalLayer presentation probe: %s\n",
      created ? "created" : "failed");
  return created ? JNI_TRUE : JNI_FALSE;
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_pebbles_1boon_metalrender_nativebridge_NativeBridge_nPresentDirectPresentationProbe(
    JNIEnv *, jclass, jfloat red, jfloat green, jfloat blue) {
  if (!g_directProbeLayer || !g_queue)
    return JNI_FALSE;
  @autoreleasepool {
    id<CAMetalDrawable> drawable = [g_directProbeLayer nextDrawable];
    if (!drawable)
      return JNI_FALSE;
...
extern "C" JNIEXPORT void JNICALL
Java_com_pebbles_1boon_metalrender_nativebridge_NativeBridge_nDestroyDirectPresentationProbe(
    JNIEnv *, jclass) {
  void (^destroyLayer)(void) = ^{
    if (g_directProbeView) {
      [g_directProbeView removeFromSuperview];
      g_directProbeView.layer = nil;
      [g_directProbeView release];
      g_directProbeView = nil;
    }
    if (g_directProbeLayer) {
      [g_directProbeLayer release];
      g_directProbeLayer = nil;
    }
  };
  if ([NSThread isMainThread])
    destroyLayer();
